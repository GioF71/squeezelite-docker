...
SQUEEZELITE_LOG_CATEGORY_ALL|-d||Support for log level on category `all` (Alsa only)
SQUEEZELITE_LOG_CATEGORY_SLIMPROTO|-d||Support for log level on category `slimproto` (Alsa only)
SQUEEZELITE_LOG_CATEGORY_STREAM|-d||Support for log level on category `stream` (Alsa only)
SQUEEZELITE_LOG_CATEGORY_DECODE|-d||Support for log level on category `decode` (Alsa only)
SQUEEZELITE_LOG_CATEGORY_OUTPUT|-d||Support for log level on category `output` (Alsa only)
SQUEEZELITE_LOG_CATEGORY_IR|-d||Support for log level on category `ir` (Alsa only)
